...
const ChatWindow = ({display,room,loggedUser,name,FetchRequests,otherUser,messagesOfRoom, setSearchBar}) => {
  ...
  const socketConnect = "http://localhost:4000";
  ...
  useEffect( async() => {
	...
	useEffect(
		() => {
      console.log("Hello");
			socketRef.current = io.connect(socketConnect)
    ...
    if(messageText.message!== ''){
      ...
      if(messagesOfRoom.length === 0){
        ...
        if (parseInt(messageCount[0].count) === 1){
          ...
          const res = await fetch(`${localhost}/connections/`,
          {
            method: 'POST',
            body: connectionForm,
          });
      ...
      try{
        ...
        if(singleConnection[0].msg_status !== 'pending' ){
           ...
           const res = await fetch(`${localhost}/connections/pending/${room}/${singleConnection[0].other_user}`,
          {
            method: 'PUT',
          })
        }
