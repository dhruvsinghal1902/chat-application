...
function App() {
  ...
  const FetchRequests = {
    ...
    getConnections: async(user_id) => {
      const response = await fetch(`${localhost}/connections/${user_id}`);
    ...
    getUserById : async(user_id) => {
      const response = await fetch(`${localhost}/users/${user_id}`);
      const user = await response.json();
      console.log("User",user[0])
      return user[0];
    },
    getMessageCount : async(room_key) => {
      const response = await fetch(`${localhost}/messages/count/${room_key}`);
      const messageCount = response.json();
      return messageCount;
    },
    getConnectionStatus : async(room_key, user_id) => {
      const response = await fetch(`${localhost}/connections/${room_key}/${user_id}`);
